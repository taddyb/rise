...
class ReplaceAndRoute:
    ...
    def post_process(
        ...
        rise_output_path.mkdir(exist_ok=True)
        try:
        ...
        for idx, file_name in enumerate(dataset_names):
            ...
            ds.to_netcdf(
                rise_output_path
                / settings.rise_output_file.format(str(formatted_timestamp))
            )
        return {"status": "OK"}
    ...
    def create_plot_file(
        ...
        plot_file_name = f"RISE_plot_output_{json_data['lid']}_{dt_start_formatted}_{dt_end_formatted}.png"
